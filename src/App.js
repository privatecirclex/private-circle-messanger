...
export default function App() {
  ...
  const handlePhotoUpload = (e) => {
    ...
    if (saveSuccess.photo) {
      setEditingPhoto(null);
      setSaveSuccess(prev => ({ ...prev, photo: false }));
      return;
    }
    ...
    setSaveSuccess(prev => ({ ...prev, photo: true }));
  };


  // --- RENDER HELPERS ---
    const Modal = ({ title, onClose, children }) => (
    <div onClick={onClose} className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-md animate-in fade-in duration-200">
      <div onClick={(e) => e.stopPropagation()} className="bg-slate-900 border border-slate-700 w-full max-w-lg rounded-3xl overflow-hidden shadow-2xl flex flex-col max-h-[95vh]">
